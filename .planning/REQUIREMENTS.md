...
- [x] **GEM-03** (v1.8): 보석 6종의 최저 즉시구매가를 **1시간마다 독립 테이블에 기록**한다 — 수집기와 같은 토큰버킷에서 예산 획득, 재기동 멱등, **답받은 것만 적재**(즉시구매 매물 없음=가격 null / 호출 실패·레이트리밋=행 없음), 헤드라인 온디맨드 캐시 신선도 회귀 0, Core Value 0줄·additive 마이그레이션만
- [x] **GEM-04** (v1.8): `/gems`에서 **측정된 적 없는 역할 라벨(딜러/서포터)을 제거**하고(Phase 24 findings 문서 포함) 레벨 중심 표(행=레벨, 열=겁화/작열)로 재구성한다 — 겁화/작열은 값이 다른 별개 아이템이므로 구분 자체는 유지
...
흩어진 3개 화면을 **대시보드 하나로 모은다**(사용자 결정 2026-07-15). `/gems`는 보석 6개 값을 보여주려고 페이지 하나를 쓰고 있었고, `이벤트 영향`은 페이지를 옮겨가 셀렉터로 품목을 다시 고르게 했다 — 대시보드에서 카드를 누르면 바로 아래 뜨는 편이 낫다. **백엔드 계약(`GET /api/gems`·`/api/items/{id}/event-impact`)은 무변경** — 화면 재배치다.

- [x] **DASH-01** (v1.8): `/gems` 페이지를 제거하고 보석을 **대시보드 좌측 카테고리**로 흡수한다 — 누르면 티어4 보석 6종을 카드로 한 번에(레벨 하위 카테고리 없음), 목록 위에 "현재 보석 시세값" 표기, **카드는 클릭 어포던스 없음**(보석은 과거 가격이 없어 이벤트 영향을 계산할 수 없다 → 갈 곳이 없다)
- [x] **DASH-02** (v1.8): `이벤트 영향` 페이지를 제거하고 대시보드에 흡수한다 — 물품 카드를 누르면 **중앙 하단에** 그 물품의 이벤트 영향 카드가 뜬다. 상관≠인과 안내와 `insufficient_data` 정직 표기는 유지(`EventImpactCards`·`impactFormat` 재활용, 재구현 금지)
- [x] **DASH-03** (v1.8): 중앙 물품 카드를 **높이 고정 스크롤 박스**에 담아 우측 소식 패널과 정렬하고(모바일은 고정높이 해제·세로 스택), `ItemCard`에 **차트 링크**를 추가해 `/timeline?item={id}` 진입을 보존한다(카드 클릭은 이벤트 영향 선택으로 바뀌므로) — `품목 타임라인` 페이지는 **유지**
...
| GEM-03 | Phase 27 | Complete |
| GEM-04 | Phase 27 | Complete |
| DASH-01..03 | Phase 28 | Complete |
