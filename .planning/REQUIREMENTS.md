...
- [ ] **ADMINUI-01**: 관리자가 시크릿 입력 화면에서 관리자 시크릿을 입력해 콘솔에 로그인하면, 이후 관리자 요청에 `X-Admin-Secret`가 자동 첨부되고 세션이 새로고침에도 유지된다
- [ ] **ADMINUI-02**: 잘못된 시크릿이면 로그인이 거부되고(401) 명확한 오류 메시지를 보여준다
- [ ] **ADMINUI-03**: 관리자가 게임 이벤트를 목록으로 보고 등록·수정·삭제할 수 있다 (event-impact 입력)
- [ ] **ADMINUI-04**: 관리자가 워치리스트 품목을 추가·비활성화(soft-delete)·재활성화할 수 있다
- [ ] **ADMINUI-05**: 관리자가 최근 수집 실행 이력(collection_run 성공/부분/실패 카운트·시각)을 콘솔에서 확인할 수 있다
- [ ] **ADMINUI-06**: 로그인하지 않은 사용자에게는 관리자 쓰기 UI가 노출되지 않으며, 관리자는 로그아웃해 세션을 종료할 수 있다

### 대시보드 카드 (CARD)

- [ ] **CARD-01**: 대시보드 품목 카드가 물품 고유 번호 없이 아이콘·품목명·골드 가격·수집 시각만 표시한다
- [ ] **CARD-02**: 대시보드 품목 카드를 클릭하면 해당 품목이 선택된 상태로 품목 타임라인 화면으로 이동한다
