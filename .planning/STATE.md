...
|---|-------------|------|--------|-----------|
| 260723-jmc | **대시보드 진행중 이벤트를 기본 3개로 접기**(프론트만·백엔드 0줄) — 사용자 피드백 "첫 화면에 읽을 게 많다"를 배포 사이트에서 실측한 결과, 총 정보량이 아니라 **한 위젯이 페이지 높이와 컬러 면적을 독점**하는 것이 원인이었다: 콘텐츠 1,515px 중 좌 900·중앙 770px에서 끝나는데 우측 배너만 1,425px까지 이어져 하단 650px가 배너 전용이었고, 화면의 유일한 컬러 이미지 6장이 전부 거기였다. 🔑 **`MAX_ROWS=6`을 쿠폰·공지 전용으로 남기고 `COLLAPSED_EVENT_ROWS=3` 신설** — 셋이 한 상수를 공유하던 동안엔 이벤트만 줄일 수 없었다. 펼침 상한은 `MAX_ROWS` 그대로라 **펼친 화면 = 이전과 동일**(접는 것이지 버리는 게 아니다). 공식 이벤트 목록으로 내보내는 안은 기각 — 각 카드가 이미 개별 공식 링크를 갖고 있어 목록을 더 보려다 이탈시킬 이유가 없다. 실측: **1,515px → 961px(−37%), 접힘 상태 스크롤 불필요**, 좌·우 컬럼 끝 차이 525px → 21px. 검증은 `VITE_API_TARGET=https://loaket.kr npm run dev`로 운영 API 실데이터를 프록시해 수행(추적 파일 수정 0). 범위 밖: 쿠폰·공지 6행 유지(텍스트라 무게 낮음), 썸네일 크기·레이아웃(개수만 건드려 효과 격리). | 2026-07-23 | 4c75e21 | [260723-jmc-news-events-trim](./quick/260723-jmc-news-events-trim/) |
| 260720-lic | **직업 아이콘 번들 → 공식 CDN 핫링크**(저장소 public 전환 대비, 프론트만). 🔑 **`260720-fn3`의 "번들" 결정을 뒤집었다** — 그때 근거(사이트 내부 자산 `2018/obt`라 예고없이 이동 가능)는 여전히 맞지만 **전제가 바뀌었다**: 오픈 API 이용약관 「지식재산권」이 "콘텐츠를 다운로드하여 보관하거나 **보관된 자료를 타인에게 제공**하는 행위"를 위반으로 규정하고 제재는 **API 접근 권한 제한**이다. private에선 '보관'이던 게 public에선 '제공'이 된다(게다가 5개는 `fill`을 고쳐 '수정'까지 겹침). 링크가 깨지면 아이콘 자리만 비지만(`onError` 폴백) **키가 정지되면 사이트가 통째로 멈춘다** → 트레이드오프 역전. ①`public/class-icons/` 30개(328KB) 삭제, `classIconUrl()`이 `cdn-lostark.../2018/obt/assets/images/common/class/{slug}.svg` 반환 ②🔑 **흰색 5개를 파일 수정이 아니라 CSS로** — 핫링크는 원본을 못 바꾼다. 단색이므로 `brightness-0`으로 30개를 검정으로 눌러 통일 + 다크 `invert`(원본이 `#222222`든 `white`든 결과 동일, 슬러그별 예외표 불필요, 스마일게이트가 원본색을 바꿔도 안 깨짐) ③**CSP 변경 0** — `cdn-lostark`는 아이템 아이콘 때문에 이미 `img-src`에 있었다(넓혀야 했다면 전제부터 재검토). 사전실측: 30/30 200, 폴백 기존재, 원본 fill 25×`#222222`/5×`white`(로컬 수정본과 정확 일치). 검증: build 그린, 라이브 `img[src*=cdn-lostark]` 30개·로드실패 0·잔여 로컬경로 0, 필터 라이트 `brightness(0)`/다크 `brightness(0) invert(1)`, 라이트·다크 육안 30개 전부 보임(옛 흰색 5개 포함), 콘솔 0. 대가: `#222222`→`#000000` 미세 진해짐, 외부요청 +30(lazy). ⚠️ 레거시 경로라 깨질 수 있음 — 그때는 이름만 남고 레이아웃은 유지 | 2026-07-20 | d57cab3 | [260720-lic-class-icon-hotlink](./quick/260720-lic-class-icon-hotlink/) |
