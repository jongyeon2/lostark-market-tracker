...
> 로스트아크 거래소의 아이템 시세를 **10분마다 자동으로 모아 기록**하고, **"언제 어떤 게임 이벤트가 있었고 그때 값이 얼마나 움직였는지"** 를 한눈에 보여주는 웹 서비스. — 좋아하는 게임으로 만들었지만, 속을 뜯어보면 **주식·코인 시세를 모으는 파이프라인과 똑같은 구조**입니다.
...
**왜 만들었나 —** 저는 로스트아크 유저이자 신입 백엔드 개발자입니다. 좋아하는 게임을 소재로, **외부 API에서 데이터를 빠짐없이 모아 저장하고 안정적으로 서빙하는 파이프라인**을 직접 설계·구현해 엔지니어링 실력을 "설명 가능하게" 보여주려고 만든 **포트폴리오 프로젝트**입니다.

### 🎯 핵심 가치 — 수집 → 저장 → 서빙 → 상관

> **외부 마켓 API에서 시세를 빠짐없이 수집해 시계열로 쌓고, 캐시로 안정적으로 서빙한다.**

다른 모든 게 실패해도 이 **수집·저장·서빙 파이프라인**은 동작해야 합니다. 그 위에 얹는 헤드라인 기능이 **event-impact**(이벤트 전후 가격 변화율 상관)이고요. 단, 이 변화율은 "인과"가 아니라 "시점 상관"입니다 — 그 이유와 정직한 처리는 아래 [설계 결정](#-설계-결정--트레이드오프-정직한-왜)에 정리했습니다.
...
아래 3화면(데스크톱 우선 React 데모)은 모두 같은 read API를 소비하고, **데이터 출처는 실행 프로파일이 정합니다** — `dev`는 실제 거래소 Open API를 호출해 쌓은 **실데이터**, `seed`는 API 키 없이 즉시 재현되는 **합성 8일치**입니다(리뷰어 진입장벽 0). 자세한 실행은 아래 [실행 방법](#-실행-방법) 참고.
...
> `seed`는 API 키 없이 즉시 재현하는 **합성 8일치** 트랙입니다('합성'임을 숨기지 않습니다). 실수집은 아래 `dev` 프로파일.
...
자세한 실행·Vite 프록시 설명·화면별 안내는 [`frontend/README`](frontend/README.md)에 일원화돼 있습니다(단일 진실 원천). **curl로도, 브라우저로도 동일한 read API** 를 봅니다.

---

## 🤖 어떻게 만들었나 (AI 협업)

이 프로젝트는 **Claude Code + GSD(Get Shit Done) 스펙 주도(spec-driven) 워크플로우**로, 자연어로 설계·구현했습니다. 다만 "AI가 알아서" 만든 결과물이 아닙니다 —

- **아키텍처·데이터 모델·트레이드오프는 제가 결정**하고 근거를 문서로 남겼습니다(아래 [설계 결정](#-설계-결정--트레이드오프-정직한-왜)이 그 기록입니다).
- 모든 변경은 **계획 → 실행 → 검증 게이트**를 거쳐 **원자적 커밋**으로 추적되고, **Testcontainers 통합 테스트**로 검증됩니다.
- 그래서 이 저장소의 코드는 **한 줄까지 "왜 이렇게 했는지" 설명할 수 있습니다** — 이 README 하단 절반이 그 증거입니다.
...
> 📦 **이하 — 엔지니어링 상세 (면접관용 깊이)**
> 위까지가 "무엇을·왜·어떻게 만들었나"라면, 아래는 **시스템이 어떻게 동작하는지**입니다. 아키텍처·설계 결정·API 계약·프로젝트 구조를 정직하게 풀어 둡니다.
...
- **단일 인스턴스:** 단일 박스(Oracle Cloud VM · Docker Compose)로 상시 운영하며, **다중 인스턴스 HA는 v2** (프로세스 다운 시 시계열 구멍은 `event-impact` `insufficient_data` 로 정직하게 노출).
...
- **빌드·테스트:** `./gradlew build` 가 전체 통합 스위트(JUnit 5 + Testcontainers)를 실행하고, 상단 CI 배지가 그 결과를 반영합니다.
