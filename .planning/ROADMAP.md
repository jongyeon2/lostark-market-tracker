...
- 🚧 **v1.8 보석 기록 + 대시보드 통합** — Phase 27·28 (착수 2026-07-15) — (27) v1.7이 세운 **"보석 시계열 미기록" 경계를 근거와 함께 넓혀** 1시간 폴러로 **기록만 시작** + 역할 라벨 제거, (28) **`/gems`·`이벤트 영향` 두 페이지를 없애고 대시보드 하나로 통합** — 보석은 카테고리로, 이벤트 영향은 물품 카드 클릭 시 하단에
...
| 27. 보석 시세 기록 시작 + 라벨 정정 | v1.8 | 0/2 | Planned — 1시간 폴러(기록만) + 역할 라벨 제거 | — |
| 28. 대시보드 통합 (`/gems`·이벤트영향 흡수) | v1.8 | 0/? | UI-SPEC 대기 — 보석=카테고리, 이벤트영향=카드 클릭 시 하단, 중앙 스크롤 박스 | — |
