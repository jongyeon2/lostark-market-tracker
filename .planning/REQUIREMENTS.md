...
- [ ] **COUPON-01**: 관리자가 쿠폰(코드·보상·만료일)을 목록으로 보고 등록·수정·삭제할 수 있고 PostgreSQL에 영속된다 (X-Admin-Secret 게이트, 기존 이벤트 CRUD 패턴 재사용, 수집/캐시/event-impact 0줄)
- [ ] **COUPON-02**: 대시보드 뉴스 패널에 유효한(미만료) 쿠폰이 만료임박순으로 표시되고(≤6), 각 쿠폰의 코드를 원클릭 복사할 수 있다 (프론트는 백엔드 `GET /api/coupons`만 소비·로아 직접 호출 금지)
- [ ] **COUPON-03**: 만료된 쿠폰은 공개 `GET /api/coupons`에서 자동 제외되며, 쿠폰이 없거나 로딩 실패해도 대시보드가 빈 화면 없이 정직히 표시한다
