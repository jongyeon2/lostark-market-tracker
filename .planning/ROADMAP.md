...
- ✅ **v1.8 보석 기록 + 대시보드 통합** — Phase 27·28 완료 (2026-07-15) — (27) v1.7이 세운 **"보석 시계열 미기록" 경계를 근거와 함께 넓혀** 1시간 폴러로 **기록만 시작** + 역할 라벨 제거, (28) **`/gems`·`이벤트 영향` 두 페이지를 없애고 대시보드 하나로 통합** — 보석은 카테고리로, 이벤트 영향은 물품 카드 클릭 시 하단에
...
**설계 스펙**: `docs/specs/2026-07-06-dashboard-news-panel-design.md`
...
**Goal:** 사용자가 실제로 쓰는 **"스펙업 재료" 중심으로 시세 커버리지를 넓히고**, 대시보드를 maplanet식 **3열(좌 카테고리 / 중앙 물품 / 우 소식)**로 재구성하며, 이벤트 상관 분석 표현력을 **이벤트 카테고리 확장**으로 높인다. 설계: `docs/specs/2026-07-14-v1.5-market-scope-ux-design.md`.
...
### ✅ v1.8 보석 기록 + 대시보드 통합 (Phase 27·28) — COMPLETE (2026-07-15)
...
- [ ] 27-02-PLAN.md — 보석 역할 라벨 제거: findings 문서 + `GemCatalog` 상수 + `SERIES_LABEL` (GEM-04)

#### Phase 28: 대시보드 통합 — `/gems`·`이벤트 영향` 페이지 흡수

**Goal**: 흩어진 3개 화면을 **대시보드 하나로 모은다**. `/gems`(보석 6개 값을 보여주려고 페이지 하나를 쓰던 것)와 `이벤트 영향`(페이지를 옮겨가 셀렉터로 품목을 다시 고르던 것)을 **제거**하고, 대시보드에서 (a) 좌측 카테고리에 **보석**을 추가해 6종을 카드로 보여주며, (b) 중앙 물품 카드를 **높이 고정 스크롤 박스**에 담고, (c) 물품 카드를 누르면 **그 아래에 이벤트 영향 카드**가 실시간으로 뜨게 한다.
**Depends on**: Phase 23(3열 레이아웃·`CategoryNav`·`categories.ts`), Phase 26(`GET /api/gems`·`GemDtos`), Phase 10(`EventImpactCards`/`EventImpactTable`/`impactFormat`/`CorrelationBanner` — 재활용 대상), Phase 16(`ItemCard` = `/timeline?item=` 링크), Phase 25(`AnchorSource` 일평균 폴백)
**Requirements**: DASH-01, DASH-02, DASH-03
**사용자 결정(2026-07-15)**:
- (a) 보석 = 대시보드 카테고리. **레벨별 하위 카테고리 없이** 6종 한 번에 렌더. 카드 위에 "현재 보석 시세값" 표기
- (b) **보석 카드는 안 눌린다** — 보석은 과거 가격이 없어 이벤트 영향을 계산할 수 없다. 눌릴 것처럼 생겼는데 안 눌리는 것보다 애초에 눌리게 안 생긴 게 낫다(Phase 26 `/gems` 행과 같은 판단)
- (c) **`ItemCard`에 '차트' 링크 추가** — 카드 클릭은 이벤트 영향 선택으로 바뀌므로, 타임라인 진입은 카드 안 별도 링크로. **`품목 타임라인` 페이지는 유지**
- (d) 중앙 스크롤 박스 높이 = 우측 소식 박스 위 선 ~ 첫 진행중 이벤트 카드 아래 (근사 고정 높이)
**Success criteria**:
1. 좌측 카테고리에 **보석**이 있고, 누르면 티어4 보석 6종(겁화·작열 × 8/9/10)이 카드로 한 번에 표시된다 — 레벨 하위 카테고리 없음, 목록 위에 "현재 보석 시세값" 표기, 가격 없음은 정직 표기
2. **`/gems` 페이지와 상단내비 '보석' 항목이 제거**되고, 보석 값은 대시보드에서만 보인다 (`GET /api/gems` 백엔드 계약은 **무변경**)
3. **`이벤트 영향` 페이지와 상단내비 항목이 제거**되고, 대시보드에서 물품 카드를 누르면 그 물품의 이벤트 영향 카드가 **중앙 하단에** 뜬다 — 상관≠인과 안내 유지, `insufficient_data` 정직 표기 유지
4. 중앙 물품 카드가 **높이 고정 박스 안에서 휠 스크롤**되고, 우측 소식 패널과 시각적으로 정렬된다. 모바일에서는 세로 스택으로 자연 축소(고정 높이 해제)
5. `ItemCard`에 **차트 링크**가 생겨 `/timeline?item={id}`로 갈 수 있고, `품목 타임라인` 페이지는 회귀 없이 동작한다
6. **보석 카드는 클릭 어포던스가 없다** — hover elevation·cursor-pointer 없음(가짜 어포던스 금지)
7. (Core Value 가드) 수집·가격 캐시·`EventImpactService`·`price_snapshot` **0줄**, 마이그레이션 **0** — 이 phase는 화면 재배치이고 백엔드 계약을 바꾸지 않는다

**Requirements 정의(DASH):**
- **DASH-01** 보석 카테고리 흡수 — `/gems` 페이지 제거, 좌측 카테고리 `보석` 추가, 6종 카드 일괄 렌더(비클릭) → Phase 28
- **DASH-02** 이벤트 영향 흡수 — `이벤트 영향` 페이지 제거, 물품 카드 클릭 시 대시보드 하단에 영향 카드 렌더 → Phase 28
- **DASH-03** 중앙 스크롤 박스 + 차트 링크 — 물품 카드 고정높이 박스(휠 스크롤), `ItemCard` 차트 링크 신설 → Phase 28

**UI-SPEC**: `28-UI-SPEC.md` (선행 필요 — 신규 레이아웃)

**Plans**: 2/2 ✅ (28-01 보석 카테고리 · 28-02 이벤트영향+스크롤박스+차트링크). UI-SPEC: `28-UI-SPEC.md`
...
| 27. 보석 시세 기록 시작 + 라벨 정정 | v1.8 | 2/2 | Complete — 1시간 폴러(기록만) + 역할 라벨 제거 · 라이브 멱등 실증 | 2026-07-15 |
| 28. 대시보드 통합 (`/gems`·이벤트영향 흡수) | v1.8 | 2/2 | Complete — 보석=단독 카테고리, 이벤트영향=카드 클릭 시 하단, 416px 스크롤 박스, 차트 링크 | 2026-07-15 |

**v1.3 Coverage:** v1.3 requirements 20 total · 매핑 **20/20 ✓** (ADMINUI 6 + CARD 2 + REALDATA 3 + POLISH 5 + DEPLOY 4)

_v1.0/v1.1/v1.2 상세는 milestones/ 아카이브. v1.3(Phases 15–18, +17.1~17.4) 라이브 배포 완료(2026-07-13). **v1.4 CI/CD 자동화 (Phase 19) 완료(2026-07-14)** — 수동 배포를 GHCR+Tailscale 무인 파이프라인으로 전환, 첫 실배포 run #30(`916bd3a`) 성공. **v1.5 시세 범위 확장 + UX (Phases 20–23) 완료(2026-07-14)** — 스펙업 재료 커버리지 확대 + 대시보드 3열 + 이벤트 카테고리. 설계 스펙 `docs/specs/2026-07-14-v1.5-market-scope-ux-design.md`. **v1.6 이벤트 영향 신뢰성 (Phase 25) 완료(2026-07-15)**. **v1.7 경매장 보석 현재가 (Phase 24·26) 완료(2026-07-15)** — v2로 미뤄둔 `SRC-V2-01` 실현, 티어4 8~10레벨 보석만·현재가 둘러보기. **v1.8 보석 기록 + 대시보드 통합 (Phase 27·28) 완료(2026-07-15)** — (27) v1.7의 "시계열 미기록" 경계를 실측 산수(1시간 주기 = 수집의 2%)를 근거로 해제하고 **기록만 먼저 켬**(라이브 6행·재기동 멱등 실증). 보석은 `Id`가 없어 **백필 불가**(경매장에 히스토리 엔드포인트 부재)이므로 일별 표·이벤트 영향 표는 데이터 축적 후 별도 phase(GEM-05/06 후보). (28) `/gems`·`이벤트 영향` 두 페이지를 대시보드로 흡수 — 화면 4개 → **2개**(대시보드·품목 타임라인). **미푸시** — 사용자 방침대로 v1.7~v1.8을 1회 push로 배포 예정(V9 마이그레이션 포함). https://lostark-tracker.duckdns.org._
