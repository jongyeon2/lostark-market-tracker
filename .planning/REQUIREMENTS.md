...
- [x] **ITEM-03**: 4개 read 응답 DTO(item list / latest / timeline / event-impact)에 iconUrl·itemGroup·roleGroup을 노출한다.
- [x] **ITEM-04**: 수집 스케줄러·Redis 캐시·EventImpactService 계산 로직은 0줄도 변경하지 않는다(회귀 테스트로 단언).
...
- [x] **SEED-03**: seed 프로파일만으로(키 없이) 신규 품목과 아이콘이 채워진다(브라우저 확인 기준).
...
- [x] **ICON-02**: Dashboard 품목 카드에 아이콘을 표시한다.
- [x] **ICON-03**: 품목 셀렉터(ItemSelect) 옵션에 아이콘을 표시한다.
- [x] **ICON-04**: Item Timeline 최신가 카드에 아이콘을 표시한다.
...
| ICON-03 | Phase 14 | Complete |
| ICON-04 | Phase 14 | Complete |
