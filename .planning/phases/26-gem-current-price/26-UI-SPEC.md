...
└─ GemPage
   └─ <Card>                          ← NewsPanel과 동일한 단일 카드 표면
      ...
      │   └─ 부제       "최저 즉시구매가 · N분마다 갱신"   ← 실제 TTL을 백엔드 계약에서 받아 표기
...
| 상태 | 표시 |
|---|---|
| pending | 6행 자리에 `Skeleton`(고정 높이 — 시프트 0) |
...
카드 헤더 부제에 **"최저 즉시구매가 · N분마다 갱신"**, 그리고 데이터 로드 후 **"기준 시각 {formatKst}"**를
헤더 우측 또는 부제 옆에 표시. 가격이 캐시 스냅샷이라는 사실을 숨기지 않는다.
`formatKst` 재사용(UTC ISO → KST) — 뉴스/쿠폰 날짜처럼 오프셋 없는 로컬 문자열이 **아니라** 서버가
`Instant`로 주는 값이므로 `formatKst`가 정답이다(D-11 함정 반대편).
...
| 부제 | **최저 즉시구매가 · N분마다 갱신** | 무엇을 재는 값인지 + 신선도를 한 줄에 |
| 그룹 헤더 | **겁화 (딜러)** / **작열 (서포터)** | 로아 유저 어휘. 역할 병기는 비유저 면접관도 읽히게 |
| 행 라벨 | **8레벨** / **9레벨** / **10레벨** | 계열은 헤더가 말하므로 중복 제거 |
| 가격 보조 라벨 | **최저 즉시구매가** | 거래소의 "최저가"와 **구별**되는 어휘 — 경매장은 즉시구매 개념이 별도로 있다 |
