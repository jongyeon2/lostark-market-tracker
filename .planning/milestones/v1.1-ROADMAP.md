# Milestone Archive: v1.1 Frontend Demo Dashboard

**Status:** ✅ SHIPPED 2026-06-29
**Phases:** 7–11
**Total Plans:** 19 (Phase 7: 3, Phase 8: 3, Phase 9: 5, Phase 10: 4, Phase 11: 4)

이 파일은 v1.1 종료 시점의 ROADMAP 스냅샷이다. 현재 활성 로드맵은 `.planning/ROADMAP.md` 참조.
...
**Delivered:** v1.0의 read API 5종을 백엔드 0줄 변경(Vite 프록시 dev 동일 출처)으로 소비하는 React + TypeScript + Tailwind + Recharts 브라우저 대시보드 3화면(Dashboard / Item Timeline / Event Impact)을, seed 프로파일 기준 빈 화면 없이 재현 가능하게 완성. README의 curl 데모를 클릭 가능한 데모 표면으로 전환.

**Key Decisions:**
- 차트 라이브러리 = **Recharts** (선언형 `<LineChart>`+`<ReferenceLine>`로 이벤트 세로 마커 자연스러움)
- 백엔드 연동 = **Vite 프록시 (dev, 백엔드 무변경)** — 단일 출처 정적 서빙(DEMO-03)은 선택/stretch로 분리, v2 강등
- 시각 = **UTC 데이터 유지 + KST 표시** (v1.0 off-by-9h 가드를 UI까지 연장; 차트 위치는 UTC instant, 라벨만 KST)
- 데이터 기준 = **seed 프로파일 우선** (합성 8일 스냅샷 + 데모 이벤트로 실키 없이 3화면 재현)
- 디자인 시스템 = **shadcn/ui (slate/new-york)** + zod-as-single-source DTO 스키마 + `.parse`-at-boundary 타입드 클라이언트 + TanStack Query fetch-on-mount

**Issues Resolved:**
- UAT Test 3 (seed 데모 헬스 카드 "12/12 실패·인증오류") — gap closure 11-04로 닫음. `SyntheticDemoData.seed()`가 합성 SUCCESS `collection_run`을 멱등 적재(startedAt=gridNow가 영속 볼륨의 과거 키리스 AUTH_ERROR run을 덮음). 읽기 경로·DTO·프론트·스키마 0줄 변경, Testcontainers 회귀 단언으로 고정.

**Technical Debt / Deferred to v2:**
- **DEMO-03** (Spring 정적 서빙으로 단일 출처 `docker compose up` 패키징) — 선택/stretch였고 슬립, v2 후보(FE-V2-04)
- 관리자 쓰기 UI(FE-V2-01), 실시간 갱신(FE-V2-02), 다크모드·i18n(FE-V2-03), 실배포·프론트 CI(FE-V2-04), 카테고리 베이스라인 초과상승률 시각화(FE-V2-05)
