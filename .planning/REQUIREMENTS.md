...
- [ ] **DASH-01** (v1.8): `/gems` 페이지를 제거하고 보석을 **대시보드 좌측 카테고리**로 흡수한다 — 누르면 티어4 보석 6종을 카드로 한 번에(레벨 하위 카테고리 없음), 목록 위에 "현재 보석 시세값" 표기, **카드는 클릭 어포던스 없음**(보석은 과거 가격이 없어 이벤트 영향을 계산할 수 없다 → 갈 곳이 없다)
- [ ] **DASH-02** (v1.8): `이벤트 영향` 페이지를 제거하고 대시보드에 흡수한다 — 물품 카드를 누르면 **중앙 하단에** 그 물품의 이벤트 영향 카드가 뜬다. 상관≠인과 안내와 `insufficient_data` 정직 표기는 유지(`EventImpactCards`·`impactFormat` 재활용, 재구현 금지)
- [ ] **DASH-03** (v1.8): 중앙 물품 카드를 **높이 고정 스크롤 박스**에 담아 우측 소식 패널과 정렬하고(모바일은 고정높이 해제·세로 스택), `ItemCard`에 **차트 링크**를 추가해 `/timeline?item={id}` 진입을 보존한다(카드 클릭은 이벤트 영향 선택으로 바뀌므로) — `품목 타임라인` 페이지는 **유지**
